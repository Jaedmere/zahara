                                        ...
                                        <div class="overflow-x-auto">
                                            ...
                                            <table class="w-full min-w-[800px] text-left text-xs">
                                                <thead class="bg-slate-50 border-b border-slate-200 font-bold text-slate-500 uppercase">
                                                    <tr>
                                                        <th class="px-4 py-3 w-8 text-center"><input type="checkbox" @change="toggleSelectAll" :checked="allSelected" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"></th>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('carteraEdsManager', () => ({
            ...
            get exportUrl() { 
                ...
                const params = new URLSearchParams({ q_factura: this.filters.q_factura, corte_desde: this.filters.corte_desde, corte_hasta: this.filters.corte_hasta });
            ...
            loadCartera(page) {
                ...
                const params = new URLSearchParams({ page: page, per_page: 50, q_factura: this.filters.q_factura, corte_desde: this.filters.corte_desde, corte_hasta: this.filters.corte_hasta });
                fetch(`${url}?${params.toString()}`).then(r => r.json()).then(response => {
