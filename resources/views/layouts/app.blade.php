...
{{-- 
    ...
    Maneja el Sidebar (Móvil), las Notificaciones (Toast) y los Modales de Confirmación.
--}}
<body class="bg-[#F8FAFC] font-sans text-slate-600 antialiased min-h-screen flex flex-col md:flex-row overflow-x-hidden"
      ...
      @modal-confirm.window="
          ...
          modal.message = $event.detail.message;
          modal.type = $event.detail.type || 'danger';
          modal.action = $event.detail.action; 
          modal.show = true;
      ">
    ...
    <aside class="fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-slate-100 shadow-2xl md:shadow-none transform transition-transform duration-300 md:translate-x-0 flex flex-col h-full"
        ...
        <nav class="flex-1 overflow-y-auto p-4 space-y-1 no-scrollbar">
            @php
                // Helper para links simples
                $navItem = function($route, $label, $icon) {
                    $active = request()->routeIs($route.'*');
                    $classes = $active ? 'bg-indigo-50 text-indigo-700 shadow-sm ring-1 ring-indigo-100' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900';
                    return '<a href="'.route($route).'" class="flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all group '.$classes.'"><span class="'.($active ? 'text-indigo-600' : 'text-slate-400 group-hover:text-slate-600').'">'.$icon.'</span>'.$label.'</a>';
                };
                ...
                $iSeguimiento = '<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>';
            @endphp
            ...
            <div x-data="{ open: {{ request()->routeIs('cartera*') || request()->routeIs('cartera_eds*') || request()->routeIs('cartera_cuentas*') ? 'true' : 'false' }} }" class="space-y-1">
                <button @click="open = !open" 
                        ...
                        :class="open || {{ request()->routeIs('cartera.*') || request()->routeIs('cartera_eds.*') || request()->routeIs('cartera_cuentas.*') ? 'true' : 'false' }} ? 'bg-slate-50 text-slate-900' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'">
                    <div class="flex items-center gap-3">
                        <span :class="open || {{ request()->routeIs('cartera.*') || request()->routeIs('cartera_eds.*') || request()->routeIs('cartera_cuentas.*') ? 'true' : 'false' }} ? 'text-indigo-600' : 'text-slate-400 group-hover:text-slate-600'">
                            {!! $iCartera !!}
                        </span>
            ...
            {!! $navItem('seguimientos.index', 'Seguimientos CRM', $iSeguimiento) !!}
    ...
    <main class="md:ml-72 flex-1 flex flex-col transition-all duration-300 min-h-screen">
        ...
        <div class="flex-1 p-4 md:p-8 w-full max-w-7xl mx-auto">
            ...
            <!-- ALERTAS DE SESIÓN LEGACY (Solo si no usa toast) -->
    ...
    <!-- COMPONENTE GLOBAL: MODALES Y TOASTS -->
