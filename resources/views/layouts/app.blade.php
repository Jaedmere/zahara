    ...
    <aside class="fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-slate-100 shadow-2xl md:shadow-none transform transition-transform duration-300 md:translate-x-0 flex flex-col h-full"
        ...
        <div class="h-20 flex-none flex items-center justify-between px-6 border-b border-slate-50">
            ...
            <a href="{{ route('notificaciones.index') }}"
               class="relative p-2 text-slate-400 hover:text-indigo-600 transition-colors hidden md:block"
        ...
        <nav class="flex-1 overflow-y-auto p-4 space-y-1 no-scrollbar">
            @php
                $navItem = fn($r, $l, $i) =>
                    '<a href="'.route($r).'" class="flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all group '
                    .(request()->routeIs($r.'*')
                ...
                $iReportes = '<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h10M4 18h6"/></svg>';
            @endphp
            ...
            {!! $navItem('eds.index', 'Estaciones EDS',
                '<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>') !!}

            {!! $navItem('clientes.index', 'Clientes',
            ...
            <div x-data="{ open: {{ request()->routeIs('cartera*') || request()->routeIs('cartera_eds*') || request()->routeIs('cartera_cuentas*') ? 'true' : 'false' }} }" class="space-y-1">
                <button @click="open = !open"
                    ...
                    <div class="flex items-center gap-3">
                        <span :class="open ? 'text-indigo-600' : 'text-slate-400 group-hover:text-slate-600'">{!! $iCartera !!}</span>
                ...
                <div x-show="open" x-collapse x-cloak class="pl-11 pr-2 space-y-1">
                    ...
                    <a href="{{ route('cartera_eds.index') }}"
                       class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors {{ request()->routeIs('cartera_eds.index') ? 'text-indigo-700 font-medium bg-indigo-50' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100' }}">
                        <svg class="w-4 h-4 opacity-70" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('cartera_cuentas.index') }}"
                       class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors {{ request()->routeIs('cartera_cuentas.index') ? 'text-indigo-700 font-medium bg-indigo-50' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100' }}">
                        <svg class="w-4 h-4 opacity-70" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            {!! $navItem('facturas.index', 'Cuentas',
                '<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>') !!}

            {!! $navItem('abonos.index', 'Abonos / Recibos',
            ...
            {!! $navItem('seguimientos.index', 'Seguimientos CRM',
                '<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>') !!}

            {{-- MENÚ REPORTES --}}
            <div class="mt-6 px-3 mb-1 text-[11px] font-bold text-slate-400 uppercase tracking-widest">
                Reportes
            </div>
            ...
            <div x-data="{ open: {{ request()->routeIs('informes.extracto') ? 'true' : 'false' }} }" class="space-y-1">
                <button @click="open = !open"
                        ...
                        :class="open ? 'bg-slate-50 text-slate-900' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'">
                    <div class="flex items-center gap-3">
                        <span :class="open ? 'text-indigo-600' : 'text-slate-400 group-hover:text-slate-600'">{!! $iReportes !!}</span>
                        <span>Reportes</span>
                    </div>
                    <svg class="w-4 h-4 text-slate-400 transition-transform duration-200"
                         :class="open ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div x-show="open" x-collapse x-cloak class="pl-11 pr-2 space-y-1">
                    <a href="{{ route('informes.extracto') }}"
                       class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors {{ request()->routeIs('informes.extracto') ? 'text-indigo-700 font-medium bg-indigo-50' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100' }}">
            ...
            {!! $navItem('users.index', 'Usuarios',
                '<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>') !!}

            {!! $navItem('roles.index', 'Roles y Permisos',
