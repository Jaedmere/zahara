...
@section('page_actions')
    <a href="{{ route('eds.create') }}" class="btn-primary px-4 py-2 text-sm inline-flex items-center gap-2">
...
<div x-data="searchHandler()" class="flex flex-col gap-6">
    ...
    <!-- BARRA DE HERRAMIENTAS (Buscador + Filtro Estado) -->
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex p-1 bg-slate-200/60 rounded-xl self-start md:self-auto">
            ...
            <a href="{{ route('eds.index', ['status' => 'active']) }}" 
               class="px-4 py-1.5 rounded-lg text-xs font-semibold transition-all {{ $status === 'active' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
                Activas
            </a>
            <a href="{{ route('eds.index', ['status' => 'inactive']) }}" 
               class="px-4 py-1.5 rounded-lg text-xs font-semibold transition-all {{ $status === 'inactive' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
                Inactivas
            </a>
        </div>
    </div>
...
<script>
    function searchHandler() {
        return {
            ...
            controller: null,
            performSearch() {
                // ... (mismo script de antes, la magia está en que respeta el parametro status de la URL) ...
                const self = this;
                if (this.controller) this.controller.abort();
                this.controller = new AbortController();
                this.isLoading = true;

                // Capturamos params actuales para mantener el filtro de estado
