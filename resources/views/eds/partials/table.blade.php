<div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden w-full">
    ...
    <div class="md:hidden space-y-3 p-4">
        @forelse($eds as $e)
            <div class="bg-white p-4 rounded-xl border border-slate-100 shadow-sm relative overflow-hidden active:scale-[0.98] transition-transform group">
                ...
                <div class="flex gap-2">
                    ...
                    @if($e->activo)
                        <form action="{{ route('eds.destroy', $e) }}" method="POST" 
                              x-on:submit.prevent="$dispatch('confirm-action', { form: $el, title: 'Desactivar', message: '¿Desactivar la estación {{ $e->nombre }}?' })"
                              class="flex-none">
                            @csrf @method('DELETE')
    ...
    <div class="hidden md:block overflow-x-auto w-full">
        <table class="w-full min-w-full text-left border-collapse table-fixed">
            <thead class="bg-slate-50 text-[10px] uppercase tracking-widest text-slate-500 font-bold border-b border-slate-200">
                <tr>
                    <th class="px-6 py-4 w-2/12">Código</th>
                    <th class="px-6 py-4 w-4/12">Estación</th>
                    <th class="px-6 py-4 w-3/12">Ubicación</th>
                    <th class="px-6 py-4 w-2/12">NIT</th>
                    <th class="px-6 py-4 w-1/12 text-right"></th>
                </tr>
            </thead>
                    ...
                    <tr class="group hover:bg-indigo-50/40 transition-colors duration-200">
                        ...
                        <td class="px-6 py-3">
                            <div class="flex flex-col">
                                ...
                                @if(!$e->activo)
                                    <span class="text-[9px] text-red-400 font-bold uppercase mt-0.5">Inactiva</span>
                                @endif
                            </div>
                        </td>
                        <td class="px-6 py-3">
                            <div class="flex flex-col text-xs text-slate-500">
                                <span class="font-medium">{{ $e->ciudad ?? '---' }}</span>
                                <span class="text-[10px] text-slate-400 truncate" title="{{ $e->direccion }}">{{ $e->direccion }}</span>
                            </div>
                        </td>
