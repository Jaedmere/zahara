                ...
                <div class="bg-white p-4 rounded-xl border border-slate-100 shadow-sm relative overflow-hidden active:scale-[0.98] transition-transform cursor-pointer"
                    ...
                    <div class="flex justify-between items-start mb-2">
                        <div class="flex-1 min-w-0 pr-2">
                            <span class="text-[10px] font-bold text-indigo-600 uppercase tracking-wider bg-indigo-50 px-1.5 py-0.5 rounded">{{ $i->consecutivo }}</span>
                            <h3 class="font-bold text-slate-800 text-sm mt-1 truncate">{{ $i->cliente->razon_social }}</h3>
                            <p class="text-xs text-slate-500 font-mono uppercase">{{ $i->eds->nombre }}</p>
                        </div>
                        <div class="text-right">
                            <span class="block font-mono font-bold text-lg text-slate-800">${{ number_format($i->saldo_pendiente, 0, ',', '.') }}</span>
                    ...
                    <div class="flex items-center justify-between border-t border-slate-50 pt-2 mt-2">
                         <div class="flex flex-col">
                             ...
                             <span class="text-xs font-medium text-slate-600">{{ $i->fecha_vencimiento->format('d/m/Y') }}</span>
                         </div>
                         ...
                         <span class="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold 
                            ...
                            @if($dias > 0) {{ $dias }} días mora @else Al día @endif
                         </span>
                    </div>
                </div>
            @empty
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($items as $i)
                        <tr class="group hover:bg-slate-50 transition-colors cursor-pointer" 
                            ...
                            <td class="px-6 py-4">
                                <span class="font-mono font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded text-xs">{{ $i->consecutivo }}</span>
                                <div class="text-[10px] text-slate-400 mt-1">{{ $i->prefijo }}</div>
                            </td>
                            <td class="px-6 py-4 truncate">
                                <div class="font-bold text-slate-700 text-sm truncate" title="{{ $i->cliente->razon_social }}">{{ $i->cliente->razon_social }}</div>
                                <div class="text-[10px] text-slate-500 font-mono uppercase">{{ $i->eds->nombre }}</div>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="text-slate-600 font-medium text-xs">{{ $i->fecha_vencimiento->format('Y-m-d') }}</span>
                                <div class="text-[9px] text-slate-400 mt-0.5">Corte: {{ $i->corte_hasta->format('d/m') }}</div>
                            </td>
                            <td class="px-6 py-4 text-center">
                                @php $dias = $i->dias_mora_calc; @endphp
                            ...
                            <td class="px-6 py-4 text-right">
                                <span class="block font-mono font-bold text-slate-800 text-sm">${{ number_format($i->saldo_pendiente, 2) }}</span>
                            </td>
                            <td class="px-6 py-4 text-right">
                    ...
                    @empty
                        <tr><td colspan="6" class="text-center py-12 text-slate-400 text-sm">No se encontraron cuentas pendientes.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
