...
<div class="flex flex-col gap-6" 
    ...
    <div class="flex flex-col md:flex-row justify-between gap-4">
        ...
        <div class="flex items-center gap-2 self-start md:self-auto">
            {{-- FILTRO EDS --}}
            <div class="relative">
                <select x-model="filters.eds_id" @change="performSearch()" class="input-pill text-xs h-10 pl-3 pr-8 appearance-none cursor-pointer shadow-sm hover:border-indigo-300 transition-colors w-full md:w-auto bg-white">
    ...
    <!-- PANEL LATERAL (Mismo código de antes) -->
                            ...
                            <div class="relative flex-1 bg-slate-50 flex flex-col min-h-0 p-6 overflow-y-auto">
                                <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-4 space-y-4">
                                    <template x-for="(item, index) in cartera" :key="item.id">
                                        <div>
                                            <div class="flex justify-between text-sm mb-1"><span class="text-slate-500">Cliente</span><span class="font-bold text-slate-800 text-right" x-text="item.cliente_nombre"></span></div>
                                            <div class="flex justify-between text-sm mb-1"><span class="text-slate-500">Estación EDS</span><span class="font-medium text-slate-700 text-right" x-text="item.eds_nombre"></span></div>
                                            <div class="flex justify-between text-sm mb-1"><span class="text-slate-500">Vencimiento</span><span class="font-mono text-red-500 text-right" x-text="item.fecha_vencimiento"></span></div>
                                            <div class="flex justify-between text-sm mb-3 pb-3 border-b border-slate-100"><span class="text-slate-500">Días Mora</span><span class="font-bold text-right" :class="item.dias_vencidos > 0 ? 'text-red-600' : 'text-emerald-600'" x-text="item.dias_vencidos > 0 ? item.dias_vencidos + ' días' : 'Al día'"></span></div>
                                            <div class="flex justify-between text-xs text-slate-400 mb-1"><span>Valor Original</span><span x-text="formatMoney(item.valor_total)"></span></div>
                                            <div class="flex justify-between text-xs text-slate-400 mb-1"><span>Abonos Previos</span><span class="text-emerald-600" x-text="formatMoney(item.abonos_previos)"></span></div>
                                            <div class="mt-4 pt-4 border-t border-slate-100">
                                                <label class="block text-xs font-bold text-indigo-600 uppercase mb-1">Monto a Abonar</label>
                                                <div class="relative"><span class="absolute left-3 top-2 text-slate-400 font-bold">$</span><input type="number" step="0.01" x-model.number="selectedMap[item.id]" class="w-full bg-indigo-50 border border-indigo-200 rounded-xl pl-7 pr-3 py-2 font-mono font-bold text-indigo-900 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-lg" :max="item.saldo_pendiente"></div>
                                                <p class="text-[10px] text-slate-400 mt-1 text-right">Máximo: <span x-text="formatMoney(item.saldo_pendiente)"></span></p>
                                                <input type="hidden" :name="`detalles[0][factura_id]`" :value="item.id"><input type="hidden" :name="`detalles[0][abono]`" :value="selectedMap[item.id]">
                                            </div>
                                        </div>
                                    </template>
                                </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('carteraCuentasManager', () => ({
            ...
            performSearch() {
                const params = new URLSearchParams({ search: this.search, ajax: '1', eds_id: this.filters.eds_id });
                window.history.replaceState({}, '', `${window.location.pathname}?${params.toString()}`);
                fetch(`${window.location.pathname}?${params.toString()}`, { headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'text/html' } })
            ...
            toggleSelection(item) {
                if (this.isSelected(item.id)) { delete this.selectedMap[item.id]; this.allSelected = false; } 
            ...
            formatMoney(v) { return '$' + parseFloat(v||0).toLocaleString('en-US', {minimumFractionDigits: 2}); },
            submitPayment(e) {
