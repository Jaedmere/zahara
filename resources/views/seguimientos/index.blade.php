...
<div class="flex flex-col gap-6 w-full max-w-full"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 animate-enter">
        @if($countHoy > 0)
        <div class="bg-indigo-50 border border-indigo-100 p-4 rounded-xl flex items-center justify-between shadow-sm cursor-pointer hover:bg-indigo-100 transition-colors"
             @click="filterStatus('hoy')">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h4 class="font-bold text-indigo-900 text-sm">Compromisos para Hoy</h4>
                    <p class="text-xs text-indigo-600">Tienes {{ $countHoy }} clientes por gestionar.</p>
                </div>
            </div>
        ...
        @if($countVencidos > 0)
        <div class="bg-red-50 border border-red-100 p-4 rounded-xl flex items-center justify-between shadow-sm cursor-pointer hover:bg-red-100 transition-colors"
             @click="filterStatus('vencidos')">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h4 class="font-bold text-red-900 text-sm">Compromisos Vencidos</h4>
                    <p class="text-xs text-red-600">{{ $countVencidos }} gestiones urgentes.</p>
                </div>
            </div>
    ...
    <div class="flex flex-col md:flex-row justify-between gap-4 w-full sticky top-0 z-20 bg-[#F8FAFC]/95 backdrop-blur py-2 md:static md:bg-transparent md:py-0">
        ...
        <div class="flex gap-2 p-1 bg-slate-200/60 rounded-xl self-start overflow-x-auto max-w-full no-scrollbar">
             <button @click="filterStatus('todos')"
                     ...
                     :class="filter === 'todos' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'">
                 Todos
             </button>
             <button @click="filterStatus('hoy')"
                     class="px-4 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1"
                     :class="filter === 'hoy' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500 hover:text-indigo-600'">
                 <span class="w-1.5 h-1.5 rounded-full bg-current" x-show="filter!=='hoy'"></span> Hoy
             </button>
             <button @click="filterStatus('vencidos')"
                     class="px-4 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1"
                     :class="filter === 'vencidos' ? 'bg-red-600 text-white shadow-md' : 'text-slate-500 hover:text-red-600'">
                 <span class="w-1.5 h-1.5 rounded-full bg-current" x-show="filter!=='vencidos'"></span> Vencidos
             </button>
        </div>
    </div>
    ...
    <div id="results-container" class="relative min-h-[200px] w-full">
        <div x-show="isSearching"
             class="absolute inset-0 bg-white/50 z-10 backdrop-blur-[1px] rounded-2xl transition-all"
        ...
        <div class="fixed inset-0 overflow-hidden pointer-events-none">
            <div class="absolute inset-0 overflow-hidden">
                <div class="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-0 md:pl-10">
                    <div x-show="isOpen"
                         class="pointer-events-auto w-screen max-w-7xl h-full"
                        ...
                        <div class="flex h-full flex-col bg-white shadow-2xl overflow-hidden">
                            ...
                            <div class="px-6 py-6 bg-slate-900 text-white shadow-md relative z-10 flex justify-between items-start flex-none">
                                <div>
                                    <h2 class="text-lg font-bold leading-6" x-text="clienteNombre"></h2>
                                    <p class="text-slate-400 text-xs mt-1">Panel de Gestión de Cobranza</p>
                                </div>
                                <button type="button"
                                        @click="closeCrm()"
                            ...
                            <div class="flex-1 overflow-hidden flex flex-col md:flex-row">
                                ...
                                <div class="w-full md:w-1/2 bg-slate-50 border-r border-slate-200 flex flex-col h-1/3 md:h-auto">
                                    <div class="p-4 border-b border-slate-200 bg-white flex justify-between items-center sticky top-0 z-10 flex-none">
                                        ...
                                        <div class="flex items-center gap-2">
                                            <span class="text-xs bg-indigo-100 text-indigo-700 px-2 py-1 rounded-lg font-bold">
                                                Total: <span x-text="formatMoney(totalSeleccionado)"></span>
                                            </span>
                                            <button @click="toggleAll(true)" class="text-[10px] text-indigo-600 font-bold bg-indigo-50 px-2 py-1 rounded">
                                                Todas
                                            </button>
                                            <button @click="toggleAll(false)" class="text-[10px] text-slate-500 font-bold bg-slate-100 px-2 py-1 rounded">
                                                Nada
                                            </button>
                                        </div>
                                    </div>
                                        ...
                                        <div class="space-y-2">
                                            <template x-for="fac in pendientes" :key="fac.id">
                                                <label class="flex items-center gap-3 p-3 bg-white rounded-xl border shadow-sm cursor-pointer transition-all hover:border-indigo-300 relative"
                                                       :class="selectedFacturas.includes(fac.id)
                                                    ...
                                                    <div class="w-5 h-5 rounded border flex items-center justify-center transition-colors flex-shrink-0"
                                                        ...
                                                        <svg class="w-3 h-3 text-white"
                                                             ...
                                                             stroke="currentColor"
                                                             viewBox="0 0 24 24"
                                                             x-show="selectedFacturas.includes(fac.id)">
                                                        ...
                                                        <div class="flex justify-between">
                                                            <div class="flex items-center gap-1.5">
                                                                <span class="font-mono font-bold text-sm text-slate-700"
                                                                      x-text="'#' + fac.consecutivo"></span>
                                                                <span class="text-[9px] text-slate-400 uppercase tracking-wide border border-slate-100 px-1 rounded truncate max-w-[80px]"
                                                                      x-text="fac.eds"></span>
                                                            </div>
                                                            <span class="font-bold text-sm text-indigo-600"
                                                                  x-text="'$' + fac.saldo_fmt"></span>
                                                        </div>
                                                        <div class="flex justify-between items-center text-[10px] text-slate-500 mt-1">
                                                            <span x-text="fac.corte_desde + ' - ' + fac.corte_hasta"></span>
                                                            <span :class="fac.dias_mora > 0 ? 'text-red-500 font-bold' : 'text-emerald-500'"
                                                                  x-text="fac.dias_mora > 0 ? fac.dias_mora + ' días mora' : 'Al día'"></span>
                                                        </div>
                                                    </div>
                                                </label>
                                            </template>

                                            <div x-show="pendientes.length === 0 && !isLoading"
                                ...
                                <div class="w-full md:w-1/2 flex flex-col h-2/3 md:h-auto border-t md:border-t-0 bg-white">
                                    <div class="p-6 border-b border-slate-200 bg-indigo-50/30 flex-none">
                                        <div class="flex justify-between items-center mb-2">
                                            <h3 class="font-bold text-xs uppercase"
                                                :class="editMode ? 'text-orange-600' : 'text-indigo-600'"
                                                x-text="editMode ? 'Editando Gestión' : 'Nueva Gestión'">
                                            </h3>
                                        ...
                                        <form @submit.prevent="saveData" class="flex flex-col gap-3">
                                            ...
                                            <div>
                                                <textarea x-model="form.observacion"
                                                          rows="2"
                                                          class="input-pill w-full mt-1 text-sm"
                                            ...
                                            <div class="grid grid-cols-2 gap-3">
                                                <div>
                                                    ...
                                                    <input type="date"
                                                           x-model="form.fecha_compromiso"
                                                           class="input-pill w-full mt-1 text-sm">
                                                </div>
                                                <div>
                                                    <label class="text-[10px] font-bold text-slate-500 uppercase">Monto</label>
                                                    <input type="number"
                                                           x-model="form.monto_compromiso"
                                                           class="input-pill w-full mt-1 text-sm"
                                                           placeholder="$ 0">
                                                </div>
                                            </div>
                                            <button type="submit"
                                                    class="btn-primary w-full justify-center mt-2 text-xs shadow-md transition-colors"
                                                    :class="editMode ? 'bg-orange-500 hover:bg-orange-600' : 'bg-indigo-600 hover:bg-indigo-700'"
                                                    :disabled="isSaving">
                                                <span x-show="!isSaving"
                                                      x-text="editMode ? 'Actualizar' : 'Registrar'"></span>
                                    ...
                                    <div class="flex-1 overflow-y-auto p-6">
                                        <h3 class="text-xs font-bold text-slate-400 uppercase mb-4 tracking-widest">
                                            Historial
                                        </h3>
                                        ...
                                        <div class="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:h-full before:w-0.5 before:bg-slate-100">
                                            <div x-show="isLoading"
                                                 class="flex justify-center py-4 relative z-10">
                                            ...
                                            <template x-for="item in historial" :key="item.id">
                                                <div class="relative flex items-start group">
                                                    ...
                                                    <div class="absolute left-0 h-10 w-10 flex items-center justify-center rounded-full bg-white border-4 transition-colors z-10"
                                                        ...
                                                        {{-- CUMPLIDO --}}
                                                        <template x-if="item.estado === 'cumplido'">
                                                        ...
                                                        {{-- CANCELADO --}}
                                                        <template x-if="item.estado === 'cancelado'">
                                                            <svg class="w-4 h-4 text-slate-400"
                                                                ...
                                                                <path stroke-linecap="round"
                                                                      stroke-linejoin="round"
                                                                      stroke-width="2"
                                                                      d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2M12 4a8 8 0 100 16 8 8 0 000-16z"/>
                                                            </svg>
                                                        </template>
                                                    </div>
                                                    ...
                                                    <div class="pl-14 w-full group/item">
                                                        <div class="flex justify-between items-start">
                                                            <span class="text-xs font-bold text-slate-500"
                                                                  x-text="item.fecha_gestion"></span>

                                                            <div class="flex items-center gap-2">
                                                                <span class="text-[10px] text-slate-400 bg-slate-50 px-2 py-0.5 rounded-full border border-slate-100"
                                                                      x-text="item.usuario"></span>

                                                                <div class="flex gap-1 opacity-100 md:opacity-0 group-hover/item:opacity-100 transition-opacity"
                                                                     x-show="item.usuario_id == {{ Auth::id() }}">
                                                                    {{-- EDITAR --}}
                                                                    <button @click="editItem(item)"
                                                                            class="p-1 text-slate-400 hover:text-orange-500"
                                                                            title="Editar">
                                                                    ...
                                                                    {{-- CANCELAR COMPROMISO --}}
                                                                    <button @click="confirmAction('cancel', item.id)"
                                                                    ...
                                                                    {{-- ELIMINAR --}}
                                                                    <button @click="confirmAction('delete', item.id)"
                                                        ...
                                                        <div class="mt-1 p-3 rounded-xl bg-slate-50 border border-slate-100 group-hover:border-indigo-100 transition-colors">
                                                            <p class="text-sm text-slate-700 whitespace-pre-wrap"
                                                               x-text="item.observacion"></p>

                                                            <template x-if="item.facturas_afectadas !== 'General'">
                                                                <div class="mt-2 text-[10px] text-slate-400 font-mono border-t border-slate-200 pt-1">
                                                                    Facturas: <span x-text="item.facturas_afectadas"></span>
                                                                </div>
                                                            </template>

                                                            <template x-if="item.fecha_compromiso">
                                                                <div class="mt-2 pt-2 border-t border-slate-200 flex items-center gap-2 text-xs">
                                                                    <span class="font-bold"
                                                                          :class="item.estado === 'pendiente' && new Date(item.fecha_compromiso) < new Date()
                                                                              ? 'text-red-600'
                                                                              : 'text-indigo-600'">
                                                                        Compromiso:
                                                                        <span x-text="item.fecha_compromiso_human"></span>
                                                                    </span>
                                                                    <template x-if="item.monto">
                                            ...
                                            <div x-show="historial.length === 0 && !isLoading"
                                                 class="text-center text-slate-400 text-sm py-4 italic ml-10">
                                                Sin gestión registrada aún.
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        Alpine.data('crmManager', () => ({
            ...
            saveData() {
                ...
                let url = this.editMode
                    ? "{{ route('seguimientos.update', ':id') }}".replace(':id', this.editId)
                ...
                let payload = {
                    ...
                    facturas_ids: this.selectedFacturas
                };
            ...
            confirmAction(type, id) {
                ...
                if (type === 'delete') {
                    ...
                    message   = '¿Eliminar registro de forma permanente?';
                    modalType = 'danger';
                } else if (type === 'cancel') {
                    title     = 'Cancelar Compromiso';
                    message   = '¿Deseas marcar este compromiso como cancelado?';
                    modalType = 'info';
                } else { // 'check'
                    title     = 'Marcar Cumplido';
                    message   = '¿Confirmas el cumplimiento del compromiso?';
                    modalType = 'info';
                }
            ...
            executeAction() {
                ...
                if (this.pendingAction === 'delete') {
                    url    = "{{ route('seguimientos.destroy', ':id') }}".replace(':id', this.pendingId);
                    method = 'DELETE';
                } else if (this.pendingAction === 'cancel') {
                    url    = "{{ route('api.seguimientos.cancel', ':id') }}".replace(':id', this.pendingId);
                    method = 'POST';
                } else { // 'check'
                    url    = "{{ route('api.seguimientos.check', ':id') }}".replace(':id', this.pendingId);
                    method = 'POST';
                }
            ...
            toggleSelection(id) {
                if (this.selectedFacturas.includes(id)) {
                    this.selectedFacturas = this.selectedFacturas.filter(f => f !== id);
                } else {
                    this.selectedFacturas.push(id);
                }
            },

            toggleAll(s) {
                this.selectedFacturas = s ? this.pendientes.map(f => f.id) : [];
            },
            ...
            editItem(item) {
                ...
                this.form.monto_compromiso = item.monto
                    ? item.monto.toString().replace(/,/g, '')
                    : '';
