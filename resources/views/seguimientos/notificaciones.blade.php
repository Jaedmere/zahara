...
<div class="flex flex-col gap-6 w-full max-w-full"
    ...
    @forelse($alertas as $alerta)
        <div class="bg-white p-5 rounded-2xl border shadow-sm transition-all hover:shadow-md
                    flex flex-col md:flex-row md:items-center justify-between gap-4 group
                    border-l-4
                    {{ $alerta->fecha_compromiso < now()->startOfDay() ? 'border-l-red-500' : 'border-l-amber-400' }}"
             data-alert-id="{{ $alerta->id }}">

            <div class="flex items-start gap-4">
                {{-- ICONO --}}
                <div class="p-3 rounded-xl
                    {{ $alerta->fecha_compromiso < now()->startOfDay() ? 'bg-red-50 text-red-600' : 'bg-amber-50 text-amber-600' }}">
                ...
                <div>
                    <div class="flex items-center gap-2 mb-1">
                        ...
                        @else
                            <span class="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-bold uppercase tracking-wide">
                                Para Hoy
                            </span>
                        @endif
                    </div>
                    ...
                    <div class="flex flex-wrap gap-4 text-xs text-slate-500">
                        ...
                        <span class="flex items-center gap-1">
                            ...
                            <span class="font-bold text-slate-700">
                                {{ $alerta->fecha_compromiso->format('d/m/Y') }}
                            </span>
                        </span>
                        ...
                        <span class="flex items-center gap-1">
                            ...
                            <span class="font-bold text-emerald-600">
                                ${{ number_format($alerta->monto_compromiso, 0) }}
                            </span>
                        </span>
                        @endif
            ...
            <div class="flex md:flex-col gap-2 min-w-[140px]">
                ...
                {{-- Cumplido --}}
                <button
                    @click="confirmAction('check', {{ $alerta->id }})"
                ...
                {{-- Eliminar --}}
                <button
                    @click="confirmAction('delete', {{ $alerta->id }})"
                    class="p-2 text-slate-400 hover:text-red-500 transition-colors md:self-end"
                    title="Eliminar">
    ...
    Alpine.data('notificationsManager', () => ({
        ...
        executeAction() {
            ...
            if (this.pendingAction === 'delete') {
                ...
                method = 'DELETE';
                successMsg = 'Notificación eliminada.';
            } else {
                url    = "{{ route('api.seguimientos.check', ':id') }}".replace(':id', this.pendingId);
                method = 'POST';
                successMsg = 'Compromiso cumplido.';
            }
