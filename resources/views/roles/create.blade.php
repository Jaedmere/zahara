...
@section('title', 'Nuevo Rol - Zahara')
    ...
    <div class="max-w-3xl animate-enter">
        <form action="{{ route('roles.store') }}" method="POST" class="space-y-6">
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm">
                ...
                <input type="text" name="nombre" value="{{ old('nombre') }}" class="input-pill" placeholder="Ej. Administrador" required autofocus>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    @foreach($permisos as $key => $label)
                        <label class="flex items-center p-3 border border-slate-100 rounded-xl hover:bg-slate-50 cursor-pointer transition-colors">
                            <input type="checkbox" name="permisos[]" value="{{ $key }}" class="w-4 h-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500">
