                ...
                <div class="pl-2">
                    ...
                    @if($f->estado !== 'pagada' && $f->estado !== 'anulada')
                        <div class="mt-4 pt-3 border-t border-slate-50 flex gap-2 justify-end">
                            ...
                            {{-- Botón Anular (Solo si no tiene abonos) --}}
                            @if($f->saldo_pendiente == $f->valor_total)
                                <form action="{{ route('facturas.destroy', $f) }}" method="POST" 
                                      x-on:submit.prevent="$dispatch('confirm-action', { form: $el, title: 'Anular Cuenta', message: '¿Anular la cuenta #{{ $f->consecutivo }}? Esta acción es irreversible.' })">
                                    @csrf @method('DELETE')
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="border-b border-slate-200 bg-slate-50/50 text-[11px] uppercase tracking-widest text-slate-500 font-bold">
                    ...
                    <th class="px-6 py-4">Emisión / Vencimiento</th>
                    ...
                    <tr class="group hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-6 py-3">
                            <div class="flex flex-col text-xs">
                                <span class="text-slate-500">{{ $f->fecha_emision->format('d M Y') }}</span>
                                <span class="{{ $f->dias_vencidos > 0 && $f->estado !== 'pagada' ? 'text-red-600 font-bold' : 'text-slate-400' }}">
                                    Vence: {{ $f->fecha_vencimiento->format('d M') }}
                                </span>
                            </div>
                        </td>
                                ...
                                @if($f->estado !== 'pagada' && $f->estado !== 'anulada')
                                    ...
                                    @if($f->saldo_pendiente == $f->valor_total)
                                        <form action="{{ route('facturas.destroy', $f) }}" method="POST" 
                                              x-on:submit.prevent="$dispatch('confirm-action', { form: $el, title: 'Anular Cuenta', message: '¿Anular la cuenta #{{ $f->consecutivo }}? Esta acción es irreversible.' })">
                                            @csrf @method('DELETE')
                                            <button class="text-slate-400 hover:text-red-600 transition-colors p-1" title="Anular">
                                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                            </button>
                                        </form>
                                    @endif
