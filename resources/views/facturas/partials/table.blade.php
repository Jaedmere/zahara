<div class="w-full">
                    ...
                    @if($f->estado !== 'pagada' && $f->estado !== 'anulada')
                        <div class="mt-4 pt-3 border-t border-slate-50 flex gap-2 justify-end">
                            <a href="{{ route('facturas.edit', $f) }}"
                               class="p-2 bg-indigo-50 text-indigo-600 rounded-lg text-xs font-bold flex items-center gap-1">
    ...
    <!-- DESKTOP TABLE (SCROLL SOLO EN TABLA) -->
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="border-b border-slate-200 bg-slate-50/50 text-[11px] uppercase tracking-widest text-slate-500 font-bold">
                    <th class="px-6 py-4 w-40">Cuenta / Acciones</th>
                ...
                @foreach($facturas as $f)
                    <tr class="group hover:bg-slate-50 transition-colors">
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all duration-200 -translate-x-2 group-hover:translate-x-0 h-6">
                                    @if($f->estado !== 'pagada' && $f->estado !== 'anulada')
                                        ...
                                        @if($f->saldo_pendiente == $f->valor_total)
                                            <form action="{{ route('facturas.destroy', $f) }}" method="POST" 
                                                  x-on:submit.prevent="$dispatch('confirm-action', { form: $el, title: 'Anular', message: '¿Anular cuenta #{{ $f->consecutivo }}?' })">
                                                @csrf @method('DELETE')
                        ...
                        <td class="px-6 py-3 text-center">
                            @if($f->estado === 'pagada')
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-700 uppercase">
                                    Pagada
                                </span>
                            @elseif($f->estado === 'anulada')
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-slate-100 text-slate-500 uppercase line-through">
                                    Anulada
                                </span>
                            @elseif($f->dias_vencidos > 0)
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-700 uppercase">
                                    Vencida
                                </span>
                            @else
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-amber-100 text-amber-700 uppercase">
                                    Pendiente
                                </span>
                            @endif
                        </td>
