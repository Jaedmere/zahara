        ...
        <div class="flex items-center self-start md:self-auto">
            <a :href="'{{ route('cartera.export') }}?search=' + (search || '')" 
               ...
               title="Exportar Todo a Excel">
                ...
                <span class="hidden md:inline">Excel General</span>
            </a>
        </div>
    </div>
                ...
                <div class="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-0 md:pl-10">
                    <div x-show="isOpen" 
                        ...
                        <form action="{{ route('abonos.store') }}" method="POST" class="flex h-full flex-col overflow-y-scroll bg-white shadow-2xl" @submit.prevent="submitPayment">
                            ...
                            <!-- Header Panel -->
                            <div class="px-4 sm:px-6 py-6 bg-indigo-600 text-white shadow-md relative z-10">
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        <h2 class="text-lg font-bold leading-6" id="slide-over-title" x-text="clienteNombre"></h2>
                                        ...
                                        <div class="flex flex-col sm:flex-row sm:items-center gap-3 mt-2">
                                            ...
                                            {{-- AQUÍ ESTÁ EL BOTÓN EXCEL INDIVIDUAL --}}
                                            <a :href="exportUrl" target="_blank" class="inline-flex items-center gap-1 text-[10px] font-bold bg-white/10 hover:bg-white/20 border border-white/20 text-white px-3 py-1.5 rounded-lg transition-colors w-fit">
                                            ...
                                            <table class="w-full text-left border-collapse">
                                                <thead class="bg-slate-50 border-b border-slate-200">
                                                    <tr class="text-[10px] uppercase text-slate-500 font-bold whitespace-nowrap">
                                                        <th class="px-4 py-3 w-8 text-center"><input type="checkbox" @change="toggleSelectAll" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"></th>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('carteraManager', () => ({
            ...
            // COMPUTED URL EXPORT (Genera link dinámico basado en cliente y filtros)
            get exportUrl() {
            ...
            loadCartera(page) {
                ...
                const params = new URLSearchParams({ page: page, per_page: 50, eds_id: this.filters.eds, q_factura: this.filters.q_factura, corte_desde: this.filters.corte_desde, corte_hasta: this.filters.corte_hasta });
                fetch(`${url}?${params.toString()}`).then(res => res.json()).then(response => {
                    if (page === 1) { this.cartera = response.data; } else { this.cartera = [...this.cartera, ...response.data]; }
                    this.meta = response.meta;
                }).finally(() => this.isLoading = false);
            },
            isSelected(id) { return this.selectedMap.hasOwnProperty(id); },
            toggleSelection(item) {
                if (this.isSelected(item.id)) { delete this.selectedMap[item.id]; } else { this.selectedMap[item.id] = parseFloat(item.saldo_pendiente); }
                this.selectedMap = { ...this.selectedMap };
            },
            toggleSelectAll() {
                this.allSelected = !this.allSelected;
                if (this.allSelected) { this.cartera.forEach(item => { if (!this.selectedMap[item.id]) { this.selectedMap[item.id] = parseFloat(item.saldo_pendiente); } }); } 
            ...
            formatMoney(amount) { return '$' + parseFloat(amount || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}); },
            submitPayment(e) {
