...
<div x-data="dashboardBI()" x-init="init()" class="space-y-6 w-full">
        ...
        <div class="flex flex-col md:flex-row justify-between gap-4 items-center">
            <div class="flex flex-wrap gap-3 items-center w-full md:w-auto">
                ...
                <div class="flex items-center bg-slate-50 rounded-xl p-1 border border-slate-200">
                    <input type="date"
                           x-model="filters.fecha_ini"
                           @change="updateData"
                           class="bg-transparent border-none text-xs font-bold text-slate-600 focus:ring-0 p-1.5 outline-none">
                ...
                <div class="relative">
                    <select x-model="filters.eds_id"
                            @change="handleEdsChange"
                            class="pl-3 pr-8 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:ring-2 focus:ring-indigo-500 appearance-none cursor-pointer">
                        <option value="">Todas las Estaciones</option>
                        @foreach($eds_list as $e)
                            <option value="{{ $e->id }}">{{ $e->nombre }}</option>
                        @endforeach
                    </select>
                    <svg class="w-4 h-4 text-slate-400 absolute right-2 top-2.5 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                    </svg>
                </div>

                <div x-show="isLoading"
                     class="text-indigo-600 text-xs font-bold flex items-center gap-1 animate-pulse"
                     style="display: none;">
            ...
            <div class="flex gap-2 mt-2" x-show="filters.rango_mora" x-cloak x-transition>
                <button @click="clearMoraFilter()"
                        class="flex items-center gap-1 px-3 py-1 bg-indigo-600 text-white rounded-full text-[10px] font-bold hover:bg-indigo-700 transition shadow-sm group">
                    <span>Filtro:</span>
                    <span x-text="filters.rango_mora"></span> 
                    <svg class="w-3 h-3 ml-1 opacity-50 group-hover:opacity-100"
                         fill="none"
                         viewBox="0 0 24 24"
                         stroke="currentColor">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group">
            ...
            <h3 class="text-3xl font-black text-slate-800 mt-1 transition-all duration-300"
                x-text="kpis.cartera.total">...</h3>
            <div class="mt-2 text-xs text-slate-500">
                Ticket:
                <span x-text="kpis.cartera.ticket_promedio"></span>
                · Facturas:
                <span x-text="kpis.cartera.facturas_vivas"></span>
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group">
            ...
            <h3 class="text-3xl font-black text-red-500 mt-1"
                x-text="kpis.cartera.vencida">...</h3>
            <div class="mt-2 text-xs text-slate-500">
                Representa el
                <strong class="text-red-500"
                        x-text="kpis.cartera.porc_vencida + '%'"></strong>
                del total
            </div>
        </div>
        ...
        <div class="bg-slate-900 p-5 rounded-2xl shadow-lg flex flex-col justify-center text-white relative overflow-hidden">
            ...
            <div class="flex items-baseline gap-2 mt-1">
                <h3 class="text-4xl font-black tracking-tighter"
                    x-text="kpis.cartera.dias_mora_pond">0</h3>
            ...
            <div class="mt-2 w-full bg-slate-700 rounded-full h-1.5 overflow-hidden">
                <div class="bg-indigo-500 h-1.5 rounded-full transition-all duration-1000"
                     :style="`width: ${Math.min(kpis.cartera.dias_mora_pond, 100)}%`"></div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm lg:col-span-2">
            <h3 class="text-sm font-bold text-slate-700 mb-4">
                Aging de Cartera (Click para filtrar)
            </h3>
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div class="flex justify-between items-center mb-4">
                ...
                <span class="text-xs bg-indigo-50 text-indigo-600 px-2 py-1 rounded-lg font-bold">
                    ...
                    <span x-text="kpis.riesgo.concentracion + '%'"></span>
                </span>
            </div>
                        ...
                        <template x-for="e in edsRiesgo" :key="e.id">
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-3 py-2 text-right font-mono text-slate-600"
                                    x-text="formatMoney(e.total)"></td>
                                <td class="px-3 py-2 text-right font-bold">
                                    <span :class="e.porc_vencido > 50
                                                    ? 'text-red-600'
                                                    : (e.porc_vencido > 20
                                                        ? 'text-amber-600'
                                                        : 'text-emerald-600')"
                                          x-text="parseFloat(e.porc_vencido).toFixed(1) + '%'">
                                    </span>
                                </td>
                            </tr>
                        </template>
            ...
            return {
                ...
                getAgingColors(labels) {
                    ...
                    return labels.map((label, index) => {
                        return label === this.filters.rango_mora
                            ? AGING_COLORS[index]
                            : '#E2E8F0';
                    });
                },
                    ...
                    fetch(`{{ route('api.dashboard.data') }}?${params}`)
                        ...
                        .then(data => {
                            ...
                            if (chartAging) {
                                ...
                                chartAging.data.datasets[0].backgroundColor =
                                    this.getAgingColors(data.charts.aging.labels);
                                chartAging.update();
                            }
                            ...
                            this.updateChart(
                                chartTop,
                                data.charts.top_clientes.labels,
                                data.charts.top_clientes.data
                            );
                            ...
                            this.updateChart(
                                chartRecaudo,
                                data.charts.recaudo_eds.labels,
                                data.charts.recaudo_eds.data
                            );
                        })
                ...
                initCharts() {
                    ...
                    if (ctxAging) {
                        const old = Chart.getChart(ctxAging);
                        if (old) old.destroy();
                    ...
                    if (ctxRecaudo) {
                        const old = Chart.getChart(ctxRecaudo);
                        if (old) old.destroy();
                    ...
                    if (ctxTop) {
                        const old = Chart.getChart(ctxTop);
                        if (old) old.destroy();
                            ...
                            options: {
                                ...
                                onClick: (e, elements, chart) => {
                                    if (elements.length > 0) {
                                        ...
                                        if (self.filters.eds_id) {
                                            url += `&eds_id=${self.filters.eds_id}`;
                                        }
