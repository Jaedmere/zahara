...
@section('page_actions')
    {{-- CAMBIO: Botón Normal (No flotante) --}}
    {{-- En móvil ocupa el 100% del ancho (w-full) y se ve grande y claro --}}
    ...
    <div class="sticky top-0 z-20 bg-[#F8FAFC]/95 backdrop-blur py-2 md:static md:bg-transparent md:py-0 transition-all">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-3">
            ...
            <div class="relative w-full md:max-w-md group shadow-sm md:shadow-none rounded-xl">
                ...
                {{-- Input con Padding Forzado (!pl-12) --}}
                <input 
                    type="text" 
                    x-model="search"
            ...
            <div class="flex p-1 bg-slate-200/60 rounded-xl self-start overflow-x-auto max-w-full no-scrollbar">
                ...
                <a href="{{ route('clientes.index', ['status' => 'bloqueado']) }}" 
                   class="whitespace-nowrap px-4 py-2 md:py-1.5 rounded-lg text-xs font-semibold transition-all {{ $status === 'bloqueado' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
                    Bloqueados
                </a>
            </div>
        </div>
