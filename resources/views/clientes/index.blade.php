...
@section('page_actions')
    <a href="{{ route('clientes.create') }}" class="btn-primary px-4 py-2.5 text-sm inline-flex items-center justify-center gap-2 shadow-sm w-full md:w-auto transition-transform active:scale-95">
...
@section('content')
{{-- Nota: quitamos los paréntesis en x-data="searchHandler" --}}
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-3">
            ...
            <div class="flex p-1 bg-slate-200/60 rounded-xl self-start overflow-x-auto max-w-full no-scrollbar">
                ...
                <a href="{{ route('clientes.index', ['status' => 'activo']) }}" 
                   class="whitespace-nowrap px-4 py-2 md:py-1.5 rounded-lg text-xs font-semibold transition-all {{ $status === 'activo' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">Activos</a>
                <a href="{{ route('clientes.index', ['status' => 'bloqueado']) }}" 
                   class="whitespace-nowrap px-4 py-2 md:py-1.5 rounded-lg text-xs font-semibold transition-all {{ $status === 'bloqueado' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">Bloqueados</a>
            </div>
        </div>
    </div>
...
<script>
    // Usamos el evento 'alpine:init' para registrar el componente de manera segura
